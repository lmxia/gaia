...
func (n nothingSelector) Matches(_ GaiaLabels) bool          { return false }
...
// NewGaiaSelector returns a nil selector
func NewGaiaSelector() Selector {
...
func (r *Requirement) hasValue(values []string) bool {
	m := make(map[string]bool)
	for _, num := range r.strValues {
		m[num] = true
	}
...
// Matches returns true if the Requirement matches the input GaiaLabels.
// There is a match in the following cases:
// (1) The operator is Exists and GaiaLabels has the Requirement's key.
// (2) The operator is In, GaiaLabels has the Requirement's key and GaiaLabels'
//
//	value for that key is in Requirement's value set.
//
// (3) The operator is NotIn, GaiaLabels has the Requirement's key and
//
//	GaiaLabels' value for that key is not in Requirement's value set.
//
// (4) The operator is DoesNotExist or NotIn and GaiaLabels does not have the
//
//	Requirement's key.
//
// (5) The operator is GreaterThanOperator or LessThanOperator, and GaiaLabels has
//
//	the Requirement's key and the corresponding value satisfies mathematical inequality.
func (r *Requirement) Matches(ls GaiaLabels) bool {
	switch r.operator {
...
// its Requirements match the input GaiaLabels. If any
// Requirement does not match, false is returned.
func (s internalSelector) Matches(l GaiaLabels) bool {
	for ix := range s {
...
func ValidatedSelectorFromSet(ls Set) (Selector, error) {
	...
	for label, value := range ls {
		r, err := NewRequirement(label, selection.Equals, value)
		if err != nil {
			return nil, err
		}
		requirements = append(requirements, *r)
...
func SelectorFromValidatedSet(ls Set) Selector {
	...
	requirements := make([]Requirement, 0, len(ls))
	for label, value := range ls {
		requirements = append(requirements, Requirement{key: label, operator: selection.Equals, strValues: value})
	}
	// sort to have deterministic string representation
	sort.Sort(ByKey(requirements))
