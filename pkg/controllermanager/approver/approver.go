...
func (crrApprover *CRRApprover) SetParentClient() {
	parentGaiaClient, parentDynamicClient, _ := utils.SetParentClient(crrApprover.localkubeclient,
		crrApprover.localgaiaclient)
...
func (crrApprover *CRRApprover) handleClusterRR(crr *platformv1alpha1.ClusterRegistrationRequest) error {
	...
	klog.V(5).Infof("create dedicated namespace for cluster %q (%q) if needed",
		crr.Spec.ClusterID, crr.Spec.ClusterName)
	ns, err := crrApprover.createNamespaceForChildClusterIfNeeded(crr.Spec.ClusterID, crr.Spec.ClusterNamePrefix,
		crr.Spec.ClusterName)
	if err != nil {
		return err
	}

	// 2. create ManagedCluster object
	klog.Infof("create corresponding MangedCluster for cluster %q (%q) if needed", crr.Spec.ClusterID,
		crr.Spec.ClusterName)
	mc, err := crrApprover.createManagedClusterIfNeeded(ns.Name, crr.Spec.ClusterName, crr.Spec.ClusterID,
		crr.Spec.ClusterLabels, crr.Spec.Secondary)
	if err != nil {
		return err
	}

	// 3. create ServiceAccount
	klog.Infof("create service account for cluster %q (%q) if needed", crr.Spec.ClusterID,
		crr.Spec.ClusterName)
	...
	klog.Infof("bind related clusterroles/roles for cluster %q (%q) if needed",
		crr.Spec.ClusterID, crr.Spec.ClusterName)
	...
	// 5. get credentials
	klog.V(5).Infof("get generated credentials for cluster %q (%q)", crr.Spec.ClusterID,
		crr.Spec.ClusterName)
	secret, err := getCredentialsForChildCluster(context.TODO(), crrApprover.localkubeclient, retry.DefaultBackoff,
		sa.Name, sa.Namespace, crrApprover.saTokenAutoGen)
	if err != nil {
		return err
	}
...
func (crrApprover *CRRApprover) createManagedClusterIfNeeded(namespace, clusterName string, clusterID types.UID,
	...
	mc, err := crrApprover.localgaiaclient.PlatformV1alpha1().ManagedClusters(namespace).Create(context.TODO(),
		managedCluster, metav1.CreateOptions{})
	if err != nil {
		klog.Errorf("failed to create ManagedCluster for cluster %q: %v", clusterID, err)
		return nil, err
	}

	klog.V(4).Infof("successfully create ManagedCluster %s/%s for cluster %s", mc.Namespace,
		mc.Name, clusterID)
	return mc, nil
}
...
func (crrApprover *CRRApprover) createServiceAccountIfNeeded(namespace, clusterName string,
	...
	newSA, err = crrApprover.localkubeclient.CoreV1().ServiceAccounts(namespace).Create(context.TODO(),
		newSA, metav1.CreateOptions{})
	if err != nil {
		return nil, err
	}
	...
	klog.V(4).Infof("successfully create dedicated service account %s for cluster %s",
		newSA.Name, clusterID)
	return newSA, nil
}
...
func getCredentialsForChildCluster(ctx context.Context, client *kubernetes.Clientset, backoff wait.Backoff,
	...
	err := wait.ExponentialBackoffWithContext(ctx, backoff, func(ctx context.Context) (done bool, err error) {
		...
		if saTokenAutoGen {
			...
			if len(sa.Secrets) == 0 {
				lastError = fmt.Errorf("waiting for secret got populated in ServiceAccount %s/%s",
					saNamespace, saName)
				return false, nil
			}
