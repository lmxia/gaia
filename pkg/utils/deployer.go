...
func GetDeployerCredentials(ctx context.Context, childKubeClientSet kubernetes.Interface, sa string) *corev1.Secret {
	...
	wait.JitterUntilWithContext(localCtx, func(ctx context.Context) {
		...
		if saTokenAutoGenerated {
			sa, errGet := childKubeClientSet.CoreV1().ServiceAccounts(known.GaiaSystemNamespace).Get(
				ctx, sa, metav1.GetOptions{})
			if errGet != nil {
				klog.ErrorDepth(5, fmt.Errorf("failed to get ServiceAccount %s/%s: %v", known.GaiaSystemNamespace, sa, errGet))
				return
			}
	...
	err := wait.ExponentialBackoffWithContext(ctx, retry.DefaultBackoff, func() (bool, error) {
		...
		if len(resource.GetNamespace()) > 0 {
			...
			if lastError != nil && !apierrors.IsAlreadyExists(lastError) {
				klog.Errorf("create ns %s error===.%v \n", resource.GetNamespace(), lastError)
				return false, nil
			}
		}
...
func GetNetworkRequirement(ctx context.Context, dynamicClient dynamic.Interface, restMapper meta.RESTMapper,
	name, desNs string) (*appsv1alpha1.NetworkRequirement, error,
) {
...
func OffloadRBWorkloads(ctx context.Context, dynamicClient dynamic.Interface, restMapper meta.RESTMapper,
	rbLabel map[string]string,
) error {
	...
	if err != nil {
		klog.ErrorS(err, "failed to offload workloads", "Description", descName)
		return err
	}
...
func ApplyResourceBinding(ctx context.Context, localDynamicClient dynamic.Interface,
	...
	if len(allErrs) > 0 {
		err = utilerrors.NewAggregate(allErrs)
		klog.ErrorS(err, "failed to apply resourcebinding", "ResourceBinding", klog.KObj(rb))
		return err
	}
...
func AssembledUserAppStructure(com *appsv1alpha1.Component, rbApps []*appsv1alpha1.ResourceBindingApps,
	clusterName, descName string, descLabels map[string]string, delete bool,
) (*unstructured.Unstructured, error) {
	depUnstructured := &unstructured.Unstructured{}
	var err error
	comCopy := com.DeepCopy()
...
func CreatNSIdNeed(dynamicClient dynamic.Interface, restMapper meta.RESTMapper, namespace string) error {
	...
	if len(namespace) > 0 {
		...
		if errNS != nil {
			errCreate := fmt.Errorf("failed to convert ns %s error===%v", namespace, err)
			return errCreate
		}
...
func CreateRBtoParentWithRetry(ctx context.Context, gaiaClient *gaiaClientSet.Clientset, namespace string,
	...
	err := wait.ExponentialBackoffWithContext(ctx, retry.DefaultBackoff, func() (bool, error) {
		...
		rbExist, err2 := gaiaClient.AppsV1alpha1().ResourceBindings(namespace).Get(ctx, rb.GetName(), metav1.GetOptions{})
		if err2 != nil {
			lastError = err
			return false, nil
		}
	...
	if err != nil {
		klog.Errorf("failed to create ResourceBinding %q to parent cluster, ERROR: %v, err is ",
